<form wire:submit="apply">
    ...
    <div class="grid grid-cols-2 gap-4">
        <label class="flex flex-col gap-2 mt-4">
            Names *
            <x-text-input type="text" autofocus wire:model="name"
                class="block mt-2 w-full read-only:opacity-50 read-only:cursor-not-allowed" />
            @error('name')
        ...
        <label class="flex flex-col gap-2 mt-4">
            ...
            <select id="type" wire:model="type"
                class="border-gray-300 focus:border-red-800 focus:ring-red-800 rounded-md shadow-sm block mt-2 w-full">
        ...
        <label class="flex flex-col gap-2 mt-4">
            ...
            <textarea wire:model="biography" rows="5"
                class="px-3 mt-2 py-2 border font-normal rounded-lg border-slate-300 read-only:opacity-50 read-only:cursor-not-allowed"></textarea>
            @error('biography')
                <div class="text-sm text-red-500 font-normal">{{ $message }}</div>
            @enderror
        </label>
